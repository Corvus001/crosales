var TiempoDeEspera = function (arreglo, TEspera_Actual) {

    var div = document.getElementById('grafico_espera');
    var svg = d3.select('#tiempo_espera').append('svg').attr('id', 'espera_interno_redraw')
    function redraw() {
        d3.select('#espera_interno_redraw').remove();
        var svg = d3.select('#tiempo_espera').append('svg').attr('id', 'espera_interno_redraw');
        // TAMAÑO
        ...
        // DATOS RECIBIDOS
        var dataPrueba = [['2017/09/28', 245], ['2017/09/29', 279], ['2017/09/30', 265], ['2017/10/02', 243], ['2017/10/03', 250], ['2017/10/04', 265], ['2017/10/05', 190], ['2017/10/06', 298], ['2017/10/07', 256], ['2017/10/09', 242], ['2017/10/10', 278], ['2017/10/11', 400], ['2017/10/13', 300], ['2017/10/14', 287]];
        ...
        svg.append('text')
            ...
            .text('T.M. de Espera Mes en Curso');
        // DIBUJAR TEspera Actual
        svg.append('text')
            .attr('x', div.clientWidth / 2)
        ...
        svg.append('text')
            ...
            .attr('font-weight', 'bold')
            .attr('id', 'variacion_te')
            .text('Variación del último año');

        if (chart_width / menor * 24 > 52) d3.select('#xkcdx').attr('font-size', 52);
        if (chart_width / menor * 24 > 52) d3.select('#variacion_te').attr('font-size', 16);
    }
