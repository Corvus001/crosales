var gaugeNivelAtencion = function (porcentajeActual, porcentajeAnterior, objetivo) {

    var div = document.getElementById('nivel_atencion');
    var svg = d3.select('#gauge_atencion').append('svg').attr('id', 'nivel_atencion_interno_redraw');

    function redraw() {
        d3.select('#nivel_atencion_interno_redraw').remove();
        var svg = d3.select('#gauge_atencion').append('svg').attr('id', 'nivel_atencion_interno_redraw');
        // TAMAÑO
        ...
        var poai = arcs.append('path')
            .attr('fill', '#1ed760')
            .attr('d', arc({ startAngle: Math.radians(-90), endAngle: Math.radians(-90) }));
        
        poai.transition().duration(1000).attrTween('d', arcTween(Math.radians(angulo_lleno)))
        ...
        poai.on('mouseenter', function (d) { poai.attr('opacity', '0.8'); }).on('mouseleave', function (d) { poai.attr('opacity', '1'); });
