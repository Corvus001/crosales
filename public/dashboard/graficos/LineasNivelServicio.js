﻿var graficoLineasServicio = function (arreglo, titulo) {

     var div = document.getElementById('grafico_servicio');
     var svg = d3.select('#servicio_anual').append('svg').attr('id','servicio_interno_redraw');
    ...
    function redraw() {
        d3.select('#servicio_interno_redraw').remove();
        var svg = d3.select('#servicio_anual').append('svg').attr('id', 'servicio_interno_redraw');
        // TAMAÑO
        ...
        // ----------- RESALTAR VALOR AL HACER MOUSE HOVER ----------
        ...
        function mousemove() {
            ...
            focus.select("text").text(formatCurrency(d[1]) + '%, ' + d[0].substring(5));
